...
async def wrapper_overview(
    ...
    async with async_session_maker() as session:
        stmt = select(
            func.sum(
                cast(Event.timestamp >= func.now() - text("INTERVAL '1 hour'"), Integer)
            ).label("events_last_1h"),
            func.sum(
                cast(
                    Event.timestamp >= func.now() - text("INTERVAL '24 hours'"), Integer
                )
            ).label("events_last_24h"),
            func.count(func.distinct(Event.payload.op("->>")("function_name"))).label(
                "unique_functions"
            ),
        ...
        data = {
            ...
            "events_last_24h": int(row.events_last_24h) if row.events_last_24h else 0,
...
async def wrapper_top_functions(
    ...
    async with async_session_maker() as session:
        nodes_element = func.jsonb_array_elements(Event.payload["nodes"]).alias("node")
        node_name = func.jsonb_extract_path_text(nodes_element.column, "name").label(
            "name"
        )
